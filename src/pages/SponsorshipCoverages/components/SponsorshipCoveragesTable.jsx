        ...
        <table className="w-full border-collapse table-fixed">
          <thead className="bg-[#f9f7f4] border-b border-border">
            <tr className="text-on-surface-variant/90">
              <th className="p-4 text-left font-black uppercase text-[11px] tracking-widest w-[28%]">{t("orphanName")}</th>
              <th className="p-4 text-center font-black uppercase text-[11px] tracking-widest w-[14%]">{t("originalAmount")}</th>
              <th className="p-4 text-center font-black uppercase text-[11px] tracking-widest w-[14%]">{t("monthlySupport")}</th>
              <th className="p-4 text-center font-black uppercase text-[11px] tracking-widest w-[14%]">{t("supportedMonths")}</th>
              <th className="p-4 text-center font-black uppercase text-[11px] tracking-widest w-[14%]">{t("status")}</th>
              <th className="p-4 text-center font-black uppercase text-[11px] tracking-widest w-[16%]">{t("reason")}</th>
            </tr>
          </thead>
  ...
  if (!data || data.length === 0) {
    return (
      <div className="text-center py-16 text-gray-400 font-medium text-base border border-border rounded-2xl bg-white shadow-sm">
        {t("noData") || (lang === "ar" ? "لا توجد بيانات متاحة" : "No data available")}
      </div>
    );
  }
      ...
      <table className="w-full border-collapse table-fixed">
        <thead className="bg-[#f9f7f4] border-b border-border">
          <tr className="text-on-surface-variant/90">
            ...
            <th className="p-4  font-black uppercase text-[11px] tracking-widest w-[28%]">
              {t("orphanName")}
            </th>
        ...
        <tbody className="divide-y divide-border bg-white">
          {data.map((item) => (
            ...
            >
              {/* عمود اسم اليتيم والأفاتار على اليسار تماماً */}
