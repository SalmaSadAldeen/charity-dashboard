      ...
      <table className="w-full border-separate border-spacing-y-2">
        ...
        <tbody>
          {data.map((req) => (
            <tr
              key={req.id}
              className="bg-white border border-border/50 shadow-sm rounded-2xl hover:shadow-[0_8px_20px_rgba(var(--primary-rgb),0.15)] hover:border-primary/30 transition-all duration-300 group"
            >
              <td className="px-6 py-5 font-bold text-on-surface-variant flex items-center gap-3">
              ...
              <td className="px-6 py-5 text-sm text-secondary font-medium">
                {/* استخدام لون tertiary هنا لإعطاء تنوع بصري هادئ */}
